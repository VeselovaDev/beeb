...
def test_create_user(user_repo, session):
    ...
    # create a user
    user_repo.create("boop", hash_password("shoop"))
    ...
    assert user.password_hash == hash_password("shoop")
...
def test_user_login_success(user_repo):
    ...
    user_repo.create("imauser", password_hash)
    db_user = user_repo.login("imauser", password_hash)
    assert db_user.username == "imauser"


def test_user_login_no_user(user_repo):
    with pytest.raises(UserNotFound):
        user_repo.login("nosuchuser", hash_password("password"))


def test_user_login_wrong_password(user_repo):
    user_repo.create("forgetful_user", hash_password("strongpassword"))
    with pytest.raises(WrongPassword):
        user_repo.login("forgetful_user", hash_password("whatwasit"))
